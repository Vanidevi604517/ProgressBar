	...
	function barAnimation(buttonValue) {
		...
		var bar = document.getElementById("barID_"+(progress-1));
		var barValue = bar.innerHTML.replace(/[^a-zA-Z0-9 ]/g, ""); 
		
		if((Number(barValue) + Number(buttonValue)) >= 0 ) { 
			bar.style.width =  Number(barValue) + Number(buttonValue)+'%';
			bar.innerHTML = Number(barValue) + Number(buttonValue)+'%';
